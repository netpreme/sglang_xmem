#!/usr/bin/env python3
"""
swe_spill_recall.py — HBM fill → spill → recall using real SWE-bench problems (SGLang backend)
...
PAGE_SIZE          = 64          # SGLang --page-size default
KV_BYTES_PER_TOKEN = 49_152      # rough estimate; used for display only
...
BAR_QUEUE   = "\033[38;5;214m"
BAR_KV      = "\033[38;5;75m"
BAR_PREFILL = "\033[38;5;114m"
...
class MetricsSnapshot:
    ...
    ttft_sum:     float = 0.0   # sglang:time_to_first_token_seconds_sum
    ttft_count:   float = 0.0
    kv_load_time: float = 0.0   # always 0 — SGLang has no separate KV-reload latency metric
...
# ── Live stream display ─────────────────────────────────────────────────────────
class MultiLiveStream:
    CONTENT_ROWS = 3

    def __init__(self, problems: list[SWEProblem]):
        self.problems   = problems
        self.n          = len(problems)
        self.display_n  = min(4, self.n)
        term_w          = shutil.get_terminal_size((160, 40)).columns
        self.col_w      = max(30, (term_w - 9) // 2)
        self.cpu_ttft: list[Optional[float]] = [None] * self.n
        self.mt_ttft:  list[Optional[float]] = [None] * self.n
        self.cpu_text: list[str]             = [""]   * self.n
        self.mt_text:  list[str]             = [""]   * self.n
        self._rows_per = 1 + 1 + 1 + self.CONTENT_ROWS + 1
        self._total    = self.display_n * self._rows_per
        self._active   = False

    @staticmethod
    def _badge(ttft: Optional[float]) -> str:
        return f"[{ttft:>8.0f}ms]" if ttft is not None else "[  pending  ]"

    def _col_header(self, color: str, label: str, ttft: Optional[float]) -> str:
    ...
    def _redraw(self) -> None:
        ...
        for i, prob in enumerate(self.problems[:self.display_n]):
            short_id = prob.instance_id.replace("__", "/")
            max_id_w = self.col_w * 2 + 1
            if len(short_id) > max_id_w:
                short_id = short_id[:max_id_w - 1] + "…"
            sys.stdout.write(f"\033[2K  {_diff_tag(prob.difficulty)}  {BOLD}{short_id}{RESET}\n")

            cpu_hdr = self._col_header(BLUE,   "hybrid-cpu",   self.cpu_ttft[i])
            mt_hdr  = self._col_header(ORANGE, "hybrid-mtier", self.mt_ttft[i])
            sys.stdout.write(f"\033[2K  {cpu_hdr}  │  {mt_hdr}\n")
            sys.stdout.write(f"\033[2K  {'─' * self.col_w}  │  {'─' * self.col_w}\n")

            cpu_lines = textwrap.wrap(self.cpu_text[i], self.col_w) if self.cpu_text[i] else []
            mt_lines  = textwrap.wrap(self.mt_text[i],  self.col_w) if self.mt_text[i]  else []
            for j in range(self.CONTENT_ROWS):
                cl = (cpu_lines[j] if j < len(cpu_lines) else "").ljust(self.col_w)[:self.col_w]
                ml = (mt_lines[j]  if j < len(mt_lines)  else "").ljust(self.col_w)[:self.col_w]
                sys.stdout.write(f"\033[2K  {BLUE}{cl}{RESET}  │  {ORANGE}{ml}{RESET}\n")

            sys.stdout.write(f"\033[2K  {DIM}{sep}{RESET}\n")
        sys.stdout.flush()

    def on_prefill(self, idx: int, key: str, ttft_ms: float, token: str) -> None:
        if key == "cpu":
            self.cpu_ttft[idx] = ttft_ms
            self.cpu_text[idx] = token
        else:
            self.mt_ttft[idx]  = ttft_ms
            self.mt_text[idx]  = token
        if idx < self.display_n:
            self._redraw()

    def on_decode(self, idx: int, key: str, token: str) -> None:
        if key == "cpu":
            self.cpu_text[idx] += token
        else:
            self.mt_text[idx]  += token
...
async def fetch_metrics_snapshot(url: str) -> MetricsSnapshot:
    ...
    try:
        ...
        for line in r.text.splitlines():
            if line.startswith("#"):
                continue
            if "sglang:queue_time_seconds_sum" in line and "{" not in line:
                snap.queue_sum   = float(line.split()[-1])
            elif "sglang:queue_time_seconds_count" in line and "{" not in line:
                snap.queue_count = float(line.split()[-1])
            elif "sglang:time_to_first_token_seconds_sum" in line and "{" not in line:
                snap.ttft_sum    = float(line.split()[-1])
            elif "sglang:time_to_first_token_seconds_count" in line and "{" not in line:
                snap.ttft_count  = float(line.split()[-1])
    except Exception:
        pass
    return snap
...
def breakdown_ms(before: MetricsSnapshot, after: MetricsSnapshot,
    ...
    dt = after.ttft_count  - before.ttft_count
    q  = (after.queue_sum - before.queue_sum) / dq * 1000 if dq > 0 else 0.0
    t  = (after.ttft_sum  - before.ttft_sum)  / dt * 1000 if dt > 0 else 0.0
    p  = max(0.0, t - q)   # prefill = TTFT - queue (includes any KV reload)
    kv = 0.0               # SGLang has no separate KV-reload latency metric
    return q, kv, p
...
def print_problem_rows(
    ...
    for i, prob in enumerate(problems):
        ...
        print(f"  {_diff_tag(prob.difficulty)}  {short_id}")
        print(f"       {BLUE}cpu  {RESET}  {_ttft_str(ct)}  {cpu_bar}")
        print(f"       {ORANGE}mtier{RESET}  {_ttft_str(mt)}  {mt_bar}  {sp_col}{sp_str}{RESET}")
...
def print_turn_summary(
    ...
    leg = f"  {DIM}legend: {BAR_QUEUE}█{RESET}{DIM} queue  {BAR_KV}█{RESET}{DIM} (unused)  {BAR_PREFILL}█{RESET}{DIM} prefill/kv-reload{RESET}"
    print(f"\n{leg}")
    ...
    if turn == 1:
        print(f"  {DIM}ⓘ  Turn 1 fills HBM — KV written to HBM + offload (write-through){RESET}")
    elif turn == 2:
        print(f"  {DIM}ⓘ  Turn 2 evicts Turn 1 KV from HBM → original sessions now offload-only{RESET}")
                ...
                try:
                    ...
                    if delta:
                        if first:
                            ...
                            first = False
                            if on_first_token:
                                on_first_token(r.ttft_ms, delta)
                        else:
                            if on_token:
                                on_token(delta)
                        parts.append(delta)
...
async def get_gpu_blocks(url: str) -> int:
    """Return GPU capacity in PAGE_SIZE-token pages (SGLang equivalent of num_gpu_blocks)."""
    async with httpx.AsyncClient() as c:
        r = await c.get(f"{url}/metrics", timeout=10)
    for line in r.text.splitlines():
        if line.startswith("#"):
        ...
        if line.startswith("sglang:max_total_num_tokens"):
            try:
                tokens = int(float(line.split()[-1]))
                return tokens // PAGE_SIZE
            except (ValueError, IndexError):
                continue
    raise RuntimeError("Could not read sglang:max_total_num_tokens from /metrics")
...
async def _build_problem(row: dict, tps: int, token: Optional[str]) -> SWEProblem:
    ...
    fill_target  = tps * 4 - header_chars
...
async def main() -> None:
    ...
    parser.add_argument("--gpu-blocks",         type=int, default=0,
                        help="Override GPU page count (default: read from /metrics)")
    args = parser.parse_args()

    print(f"\n{BOLD}SWE-bench HBM Fill → Spill → Recall  [SGLang]{RESET}")
    ...
    num_gpu_pages = args.gpu_blocks or await get_gpu_blocks(CPU_URL)
    gpu_token_cap = num_gpu_pages * PAGE_SIZE
    tps           = min(95_000, max(500, gpu_token_cap // args.concurrency))

    if num_gpu_pages <= 2048:
        print(f"\n  {YELLOW}⚠  GPU pages={num_gpu_pages} — for full cache run: bash demo/start_servers.sh --no-cap{RESET}")

    recall_kv_mb = 2 * tps * KV_BYTES_PER_TOKEN / 1e6
    print(f"\n  GPU KV capacity  : {gpu_token_cap:,} tokens  [{num_gpu_pages} pages × {PAGE_SIZE}]")
    ...
    evict_sid = random.getrandbits(32)

    def _sys_msg(p: SWEProblem, s: int) -> dict:
        return {"role": "system", "content":
                f"[s:{s:08x}][{p.instance_id}] You are a senior software engineer. Be concise."}

    # Original sessions: fill HBM on Turn 1, recall from offload on Turn 3+
    cpu_sessions = [SessionState(p, [_sys_msg(p, sid)])       for p in problems]
    mt_sessions  = [SessionState(p, [_sys_msg(p, sid)])       for p in problems]
    # Eviction sessions: different SID → no prefix-cache hit → displaces original KV from HBM
    cpu_evict    = [SessionState(p, [_sys_msg(p, evict_sid)]) for p in problems]
    mt_evict     = [SessionState(p, [_sys_msg(p, evict_sid)]) for p in problems]
    ...
    for turn in range(1, 2 + args.recall_turns + 1):
        if turn == 1:
            ...
            active_cpu, active_mt = cpu_sessions, mt_sessions
        elif turn == 2:
            phase, max_tokens = "fill-offload",  32
            active_cpu, active_mt = cpu_evict, mt_evict
        else:
        ...
        for i in range(args.concurrency):
            if turn <= 2:
                ...
                content = RECALL_Q
            active_cpu[i].messages.append({"role": "user", "content": content})
            active_mt[i].messages.append( {"role": "user", "content": content})
        ...
        async with httpx.AsyncClient() as client:
            ...
            for i in range(args.concurrency):
                tasks.append(send_request(client, CPU_URL,   "hybrid-cpu",
                                          ...
                                          list(active_cpu[i].messages), max_tokens,
                                          on_first_token=functools.partial(live.on_prefill, i, "cpu"),
                                          on_token=functools.partial(live.on_decode, i, "cpu")))
                tasks.append(send_request(client, MTIER_URL, "hybrid-mtier",
                                          problems[i].instance_id, turn,
                                          list(active_mt[i].messages), max_tokens,
                                          on_first_token=functools.partial(live.on_prefill, i, "mt"),
                                          on_token=functools.partial(live.on_decode, i, "mt")))
        ...
        for i in range(args.concurrency):
            ...
            active_cpu[i].messages.append({"role": "assistant", "content": canonical})
            active_mt[i].messages.append( {"role": "assistant", "content": canonical})
        ...
        if turn == 1:
            ...
            print(f"\n  {DIM}Waiting 3s for HBM eviction to settle…{RESET}")
            await asyncio.sleep(3)
    ...
    if recall:
        ...
        if ct50 and mt50:
            sp_str, sp_col = _speedup_str(ct50, mt50)
            direction = "MTier faster" if ct50 / mt50 > 1.05 else ("CPU faster" if ct50 / mt50 < 0.95 else "similar")
            sp95_str, sp95_col = _speedup_str(ct95, mt95)
            def _p(v): return f"{v:>7.0f}ms" if v else "       ?"
            print(f"  {BLUE}hybrid-cpu   {RESET}  p50 {_p(ct50)}  p95 {_p(ct95)}")
            print(f"  {ORANGE}hybrid-mtier {RESET}  p50 {_p(mt50)}  p95 {_p(mt95)}")
            print(f"\n  {BOLD}Overall speedup  p50: {sp_col}{sp_str}{RESET}  {BOLD}p95: {sp95_col}{sp95_str}{RESET}  {direction}")
